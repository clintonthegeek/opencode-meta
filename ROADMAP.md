...
| P2-2 | Add search/filter to all lists | QLineEdit + proxy models for Roles, Teams, Trials, Projects tables | `RolesWidget`, `TeamsWidget`, `TrialsWidget`, `ProjectsWidget` | Medium | [x] | New `FilterBar` widget (QLineEdit + clear button + ESC shortcut + `filterChanged` signal) + `FilterProxyModel` (subclass of QSortFilterProxyModel with public `acceptsRow`); each list hosts a `FilterBar` directly above the table/list and a proxy that maps its accept/reject onto `setRowHidden` / `item.setHidden` so existing item()/currentRow()/currentItem() call sites stay untouched; case-insensitive search across every visible column via `setFilterKeyColumn(-1)`; selection/action wiring explicitly skips hidden rows in `selectedRoleId`/`selectedTeamId`/`selectedTrialIds`/`currentProject`, and `onSelectionChanged` re-runs as part of `applyFilter` so disabled-button state stays consistent. New `test_list_filter` covers all four widgets (case-insensitive name/id/description/mode match for Roles; id/name/description for Teams; team name + notes for Trials; project-path summary for Projects and propagated button-disable). Side fix to `TeamsWidget::refreshTeams` preserves the user's current selection by id (not row index) across refreshes — `StorageManager::listTeams` is filesystem-order, so the cross-view smoke test was already silently applying the Variant team after Duplicate Variant because the editor flipped to the new row; the smoke test now passes alongside the new test (`ctest` 13/13 green) – 2026-06-28 |
...
**Current Overall Progress**: 13 / ~25 tasks completed (Phase 1 fully closed; P2-1 and P2-2 closed).

**Session Update (2026-06-28)**: Phase 0 high-priority tasks P0-1 & P0-2 completed by sub-agents. Progress: 11/25 after closing P1-1 (Live Rendered Config Inspector), P1-2 (Prompt Merge Preview), P1-3 (Role Editor now exposes mode/permissions/tools/metadata via a 3-tab dialog with a dedicated round-trip test), P1-4 (Inline Specialist editing via `EditSpecialistDialog`), and P1-5 (ConfirmApplyDialog — pre-apply diff gate resolving the long-standing TODO in `ProjectsWidget::switchTeamForProject`). P1 closed; P2-1 closed: `TrialCompareDialog` extracted from the TrialsWidget inline stub; full side-by-side rendered-config diff with snapshot > re-render > placeholder precedence; `ctest` 12/12 green. P2-2 closed: shared `FilterBar` (QLineEdit + clear button + ESC shortcut) + `FilterProxyModel` (QSortFilterProxyModel subclass with public `acceptsRow`) added to Roles/Teams/Trials/Projects lists; case-insensitive substring match across every visible column; selection/action wiring explicitly skips hidden rows; new `test_list_filter` covers all four widgets; `ctest` 13/13 green.

**Last updated: 2026-06-28 by @claude** (P2-2 in flight → completed)
